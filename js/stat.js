...
var scrinStat = function (ctx, x, y, color) {
  ctx.fillStyle = color;
  ctx.fillRect(x, y, WIDTH_STAT, HEIGHT_STAT);
};

window.renderStatistics = function (ctx, names, times) {
  scrinStat(ctx, STAT_X + GAP, 2 * GAP, 'rgba( 0, 0, 0, 0.7 )');
  scrinStat(ctx, STAT_X, GAP, '#fff');
  ...
  for (var i = 0; i <= names.length - 1; i++) {
    ...
    names[i] === playerName ? ctx.fillStyle = playerBarColor : ctx.fillStyle = 'hsl(240,100%,50%)';
